...
public class Source {
	...
	case 1:
	    ...
	    break;
	case 3:
	    if (Time.currentTicks() - sink.NCalcFireTime > Time.toTickSpan(Time.SECONDS, 1)){
		Logger.appendString(csr.s2b("Re-starting on Channel"));
		Logger.appendByte(channel);
		Logger.flush(Mote.WARN);
		sink.state = 0;
		return 0;
	...
	if (sink.state == 2){
	    ...
	    sink.NCalcFireTime = time + (sink.t * 10);
	    sink.state = 3;
	}
